...
def get_scheduler(optimizer, opt):
    ...
    if opt.lr_policy == 'lambda':
        ...
        def lambda_rule(epoch):
            lr_l = 1.0 - max(0, epoch + 1 + 1 + opt.iter_count -
        ...
        raise NotImplementedError('learning rate policy [%s] is not implemented',
                                  opt.lr_policy)
    return scheduler
...
class AddCoords(nn.Module):
    ...
    def forward(self, x):
        ...
        yy_cahnnel = torch.arange(y_dim).repeat(B, 1, x_dim, 1).permute(0, 1, 3,
                                                                        2).type_as(x)
        # normalization
...
class ResBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.bypass = coord_conv(input_nc, output_nc, use_spect, use_coord,
                                 **kwargs_short)
...
class ResBlockEncoderOptimized(nn.Module):
    ...
    def __init__(self,
        ...
        if type(norm_layer) == type(None):
            self.model = nn.Sequential(self.conv1, nonlinearity, self.conv2,
                                       nn.AvgPool2d(kernel_size=2, stride=2))
        else:
            self.model = nn.Sequential(self.conv1, norm_layer(output_nc), nonlinearity,
                                       self.conv2, nn.AvgPool2d(kernel_size=2, stride=2))
...
class ResBlockDecoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv1 = spectral_norm(
            nn.Conv2d(input_nc, hidden_nc, kernel_size=3, stride=1, padding=1), use_spect)
        self.conv2 = spectral_norm(
            nn.ConvTranspose2d(hidden_nc,
                               output_nc,
                               kernel_size=3,
                               stride=2,
                               padding=1,
                               output_padding=1), use_spect)
...
class Output(nn.Module):
    ...
    def __init__(self,
        ...
        if type(norm_layer) == type(None):
            self.model = nn.Sequential(nonlinearity,
                                       nn.ReflectionPad2d(int(kernel_size / 2)),
                                       self.conv1, nn.Tanh())
        else:
            self.model = nn.Sequential(norm_layer(input_nc), nonlinearity,
                                       nn.ReflectionPad2d(int(kernel_size / 2)),
                                       self.conv1, nn.Tanh())
...
class Auto_Attn(nn.Module):
    ...
    def __init__(self, input_nc, norm_layer=nn.BatchNorm2d):
        ...
        self.model = ResBlock(int(input_nc * 2),
                              input_nc,
                              input_nc,
    ...
    def forward(self, x, pre=None, mask=None):
        ...
        if type(pre) != type(None):
            ...
            context_flow = torch.bmm(pre.view(B, -1, W * H),
                                     attention.permute(0, 2, 1)).view(B, -1, W, H)
